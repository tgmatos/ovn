...
#include "en-ts.h"
#include "northd/en-lb-data.h"
#include "openvswitch/util.h"
#include "openvswitch/vlog.h"
#include "openvswitch/hmap.h"
#include "openvswitch/shash.h"
#include "ovn-ic-nb-idl.h"
...
static uint32_t allocate_dp_key(struct hmap *dp_tnlids, bool vxlan_mode,
                                const char *name);
...
en_ts_run(struct engine_node *node, void *data)
{
    ...
    stopwatch_start(OVN_IC_TS_RECOMPUTE_STOPWATCH_NAME, time_usec());
    build_ts_run(ts_table, dp_table, ls_table, nb_global_table, encap_table,
                 &ts_data->dp_tnlids, &ts_data->isb_ts_dps, &ts_data->nb_tses);
    stopwatch_stop(OVN_IC_TS_RECOMPUTE_STOPWATCH_NAME, time_usec());
...
en_ts_handler(struct engine_node *node, void *data)
{
    ...
    const struct ed_type_enum_datapaths *enum_datapath =
    ...
    ICNBREC_TRANSIT_SWITCH_TABLE_FOR_EACH_TRACKED (tracked_ts, ts_table) {
        ...
        if (icnbrec_transit_switch_is_deleted(tracked_ts)) {
            const struct icsbrec_datapath_binding *dp =
                shash_find_and_delete(
                            &datapath_tracked_data.crupdated_datapaths,
                            tracked_ts->name);
            if (dp) {
        ...
        /* Sync TS between INB and ISB.  This is performed after
         * syncing with AZ SB, to avoid uncommitted ISB datapath
         * tunnel key to be synced back to AZ.
         */
        if (ctx->ovnisb_idl_txn) {
            const struct icsbrec_datapath_binding *isb_dp =
                shash_find_and_delete(&ts_data->isb_ts_dps, tracked_ts->name);
            ...
            if (!isb_dp) {
                ...
                /* Existing ISB Datapath Binding - ensure key is correct */
                int64_t dp_key = allocate_dp_key(
                    &ts_data->dp_tnlids, vxlan_mode,
                   "transit switch datapath");
                if (dp_key) {
        ...
        struct icsbrec_datapath_binding *isb_dp =
            shash_find_data(&datapath_tracked_data.crupdated_datapaths,
                            tracked_ts->name);
        if (isb_dp) {
            ...
            if (nb_tnl_key != isb_dp->tunnel_key) {
                VLOG_DBG("Set other_config:requested-tnl-key %"PRId64
                         ...
                         isb_dp->tunnel_key, tracked_ts->name);
        ...
        if (icnbrec_ic_nb_global_is_updated(icnb_global,
            ...
            NBREC_LOGICAL_SWITCH_TABLE_FOR_EACH (ls, ls_table) {
                ...
                if (isb_dp && dp_key_refresh) {
                    ...
                    struct tnlid_node *tnl;
                    HMAP_FOR_EACH_WITH_HASH (tnl, hmap_node,
                        hash_int(isb_dp->tunnel_key, 0), &ts_data->dp_tnlids) {
                        if (tnl->tnlid == isb_dp->tunnel_key) {
                            hmap_remove(&ts_data->dp_tnlids, &tnl->hmap_node);
                            free(tnl);
                            break;
                        }
                    }
...
{
    struct tnlid_node *dp;
    HMAP_FOR_EACH_SAFE (dp, hmap_node, &data->dp_tnlids) {
        free(dp);
    }

    hmap_destroy(&data->dp_tnlids);
