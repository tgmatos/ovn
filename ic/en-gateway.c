...
static void gateway_clear_tracked(struct ed_type_gateway *data);
...
void
en_gateway_clear_tracked_data(void *data)
{
    gateway_clear_tracked(data);
}
...
static void
gateway_clear_tracked(struct ed_type_gateway *data)
{
    struct ed_type_gateway *gw_data = data;
    struct gateway_tracked *tr_data;

    HMAP_FOR_EACH_SAFE (tr_data, node, &gw_data->tracked_data) {
        hmap_remove(&gw_data->tracked_data, &tr_data->node);
        free(tr_data);
    }
    gw_data->tracked = false;
}
...
gateway_clear(struct ed_type_gateway *data)
{
    ...
    struct gateway_tracked *gw_data;
    HMAP_FOR_EACH_SAFE (gw_data, node, &data->tracked_data) {
        hmap_remove(&data->tracked_data, &gw_data->node);
        free(gw_data);
    }
