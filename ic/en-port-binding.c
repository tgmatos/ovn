...
static void port_binding_clear_tracked(struct ed_type_port_binding *data);
...
void
en_port_binding_clear_tracked_data(void *data)
{
    port_binding_clear_tracked(data);
}
...
static void
port_binding_clear_tracked(struct ed_type_port_binding *data)
{
    struct ed_type_port_binding *pb_data = data;
    struct port_binding_tracked *tr_data;

    HMAP_FOR_EACH_SAFE (tr_data, node, &pb_data->tracked_data) {
        hmap_remove(&pb_data->tracked_data, &tr_data->node);
        free(tr_data);
    }
    pb_data->tracked = false;
}
...
port_binding_clear(struct ed_type_port_binding *data)
{
    ...
    struct port_binding_tracked *pb_data;
    HMAP_FOR_EACH_SAFE (pb_data, node, &data->tracked_data) {
        hmap_remove(&data->tracked_data, &pb_data->node);
        free(pb_data);
    }
...
en_port_binding_handler(struct engine_node *node, void *data)
{
